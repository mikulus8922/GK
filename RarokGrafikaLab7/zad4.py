    ...
    vertex_shader_source = """
        ...
        void main(void) {
            gl_Position = P_matrix * V_matrix * M_matrix * (position + 
                (gl_InstanceID % 10) * vec4(1.0, 0, 0, 0) + (gl_InstanceID / 10) * vec4(0, 1.0, 0, 0));
            vertex_color = color;
        }
    """
...
def startup():
    ...
    color_array_object = glGenVertexArrays(1)
    glBindVertexArray(color_array_object)
    ...
    glBufferData(GL_ARRAY_BUFFER, color_positions, GL_STATIC_DRAW)
...
def shutdown():
    ...
    glDeleteBuffers(2, color_buffer)
